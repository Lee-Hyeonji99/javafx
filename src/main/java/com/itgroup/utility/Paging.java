...
public class Paging {
    ...
    private int pageSize = 0 ; //페이지당 건수
    ...
    public Paging(String _pageNumber, String _pageSize, int totalCount, String url, String mode, String keyword) {
        ...
        beginRow = pageSize * (pageNumber - 1) + 1;
        endRow = Math.min(totalCount, beginRow + pageSize - 1);

        // pageCount = pages per paging
        beginPage = pageCount * ((pageNumber - 1) / pageCount) + 1;

        endPage = Math.min(totalPage, beginPage + pageCount - 1);
